        ...
        loop {
            ...
            match xml.read_event_into(&mut buf) {
                Ok(Event::Start(ref e)) if e.local_name().as_ref() == b"cacheField" => {
                    ...
                    pivot_cache.push(PivotCacheDefinition {
                        cache_defintion_name: cache_name,
                        cache_defintion_string: cached_values.unwrap(),
                    });
                }
        ...
        match xml.read_event_into(&mut buf) {
            Ok(Event::Start(ref e)) if e.local_name().as_ref() == b"r" => {
                ...
                row_data
                    .iter()
                    .enumerate()
                    .for_each(|(i, v)| column_based[i].push(v.to_string()));
            ...
            Ok(Event::Start(ref e)) if e.local_name().as_ref() == b"x" => {
                ...
                e.attributes().into_iter().for_each(|a| {
                    ...
                    if att.key == QName("v".as_bytes()) {
                        ...
                        let ref str = pt[x_cnt].cache_defintion_string[index];
